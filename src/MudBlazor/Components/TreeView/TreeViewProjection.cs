...
{
    private static readonly ReadOnlyCollection<TreeViewItemContext<T>> EmptyRows = new List<TreeViewItemContext<T>>().AsReadOnly();
    ...
    /// </summary>
    public ReadOnlyCollection<TreeViewItemContext<T>> Rows { get; private set; } = EmptyRows;
        ...
        IReadOnlyCollection<T>? selectedValues)
    {
        ...
        var reconciledSelection = new HashSet<T>(currentSelection.Where(_representedValues.Contains), _comparer);
        var reconciledValue = selectedValue is not null && _representedValues.Contains(selectedValue)
            ? selectedValue
            : default;
