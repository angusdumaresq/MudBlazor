...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class TreeViewTests : BunitTest
    {
        ...
        /// <summary>
        /// Verifies that ArrowRight on an off-screen collapsed row expands it once the row renders instead of dropping the key.
        /// </summary>
        [Test]
        public async Task TreeViewVirtualize_KeyboardExpand_RunsWhenOffscreenTargetRenders()
        {
            var keyInterceptorService = Context.AddKeyInterceptorService();
            var comp = Context.Render<TreeViewVirtualizationTest>(parameters => parameters
                .Add(x => x.Height, "40px")
            ...
            comp.Find("ul.mud-treeview").GetAttribute("aria-activedescendant").Should().EndWith("_item_2");
            GetVirtualizedValues(comp).Should().NotContain("Grandchild", "the row has not rendered yet, so the expand is still queued");

            await SetVirtualizedWindowAsync(comp, beforeSpacer: true, spacerSize: 80, spacerSeparation: 40, containerSize: 40);
        ...
        /// <summary>
        /// Verifies that ArrowLeft on an off-screen expanded row collapses it once the row renders instead of dropping the key.
        /// </summary>
        [Test]
        public async Task TreeViewVirtualize_KeyboardCollapse_RunsWhenOffscreenTargetRenders()
        {
            var keyInterceptorService = Context.AddKeyInterceptorService();
            var comp = Context.Render<TreeViewVirtualizationTest>(parameters => parameters
                .Add(x => x.DeepTreeDepth, 10)
            ...
            GetVirtualizedValues(comp).Should().Contain("Node 9", "the row has not rendered yet, so the collapse is still queued");

            await SetVirtualizedWindowAsync(comp, beforeSpacer: true, spacerSize: 320, spacerSeparation: 40, containerSize: 40);

            comp.WaitForAssertion(() =>
            {
                GetVirtualizedValues(comp).Should().Contain("Node 8").And.NotContain("Node 9");
                comp.Find("ul.mud-treeview").GetAttribute("aria-activedescendant").Should().EndWith("_item_8");
            });
        }

        /// <summary>
        /// Verifies that Home scrolls the first row back into view when it is already the active row.
        /// </summary>
        /// <remarks>
        /// The active row does not change, so nothing else would schedule a render; the scroll must still happen.
        /// </remarks>
        [Test]
        public async Task TreeViewVirtualize_KeyboardNavigation_HomeRescrollsActiveRowAtBoundary()
        {
            var keyInterceptorService = Context.AddKeyInterceptorService();
            var comp = Context.Render<TreeViewVirtualizationTest>(parameters => parameters
                .Add(x => x.Height, "40px")
            ...
            AssertActiveItem(comp, "Root");
            GetScrollTargets(comp).Should().BeEmpty();

            await DispatchTreeKeyAsync(keyInterceptorService, comp, "Home");

            GetScrollTargets(comp).Should().Equal(0);
            AssertActiveItem(comp, "Root");
        }

        /// <summary>
        /// Verifies that a caller-supplied item id is kept when the tree is not virtualized.
        /// </summary>
        [Test]
        public void TreeView_ItemAttributes_PreserveCallerIdWhenNotVirtualized()
        {
            var comp = Context.Render<TreeViewVirtualizationTest>(parameters => parameters
            ...
            items.Should().OnlyContain(item => item.Id == "caller-item" && item.GetAttribute("data-caller-item") == "preserved");
        }
