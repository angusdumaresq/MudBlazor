...
public class TreeViewProjectionTests
{
    ...
    /// <summary>
    /// Verifies that backing multi-selection changes win after initialization and values removed from the hierarchy are discarded.
    /// </summary>
    [Test]
    public void ReconcileSelectionReflectsExternalMultiSelectionAndPrunesMissingValues()
    {
        ...
        replaced.SelectedValues.Should().Equal("First");
        replaced.SelectionChanged.Should().BeTrue();
    }

    /// <summary>
    /// Verifies that backing single-selection changes win after initialization and a removed selected value is cleared.
    /// </summary>
    [Test]
    public void ReconcileSelectionReflectsExternalSingleSelectionAndClearsMissingValue()
    {
        var first = new TreeItemData<string> { Value = "First" };
        var second = new TreeItemData<string> { Value = "Second" };
        var projection = new TreeViewProjection<string>();
        ...
        replaced.SelectedValue.Should().BeNull();
        replaced.SelectionChanged.Should().BeTrue();
    }
